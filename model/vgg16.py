...
class VGG16Model(nn.Module):
    def __init__(self, num_classes, conv_arch, input_channels=3):
        ...
        self.classifier = nn.Sequential( # 全连接层结构
            ...
            # 如果用的是 nn.CrossEntropyLoss() 作为损失函数，模型最后一层不需要加Softmax，因为CrossEntropyLoss内部已经包含了Softmax操作
    ...
    def make_layers(self): # 创建卷积层结构
        ...
        for num_convs, out_channels in self.conv_arch: 
            for _ in range(num_convs): # 遍历每个卷积块的层数
                layers.append(nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)) # 添加卷积层，kernel_size=3，padding=1保持输出大小不变
                layers.append(nn.ReLU(inplace=True)) # 添加ReLU激活函数
                in_channels = out_channels # 更新输入通道数为当前卷积层的输出通道数
